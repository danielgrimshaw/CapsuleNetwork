...
class CapsNet(object):
    ...
    def build_arch(self):
        with tf.variable_scope('Conv1_layer'):
            ...
            conv2 = tf.contrib.layers.conv2d(conv1, num_outputs=128,
                    kernel_size=4, stride=1, padding="VALID")
            conv3 = tf.contrib.layers.conv2d(conv2, num_outputs=192,
                    kernel_size=4, stride=1, padding="VALID")
            conv4 = tf.contrib.layers.conv2d(conv3, num_outputs=256,
                    kernel_size=4, stride=1, padding="VALID")
            assert conv4.get_shape() == [cfg.batch_size, 20, 20, 256], conv4.get_shape()

        # Primary Capsule Layer
        with tf.variable_scope('PrimaryCaps_layer'):
            primaryCaps = CapsLayer(num_outputs=32, vec_len=8, with_routing=False, layer_type="CONV")
            caps1 = primaryCaps(conv4, kernel_size=9, stride=2)
            assert caps1.get_shape() == [cfg.batch_size, 1152, 8, 1], caps1.get_shape()
        ...
        with tf.variable_scope('Masking'):
            ...
            assert self.softmax_v.get_shape() == [cfg.batch_size, 10, 1, 1], self.softmax_v.get_shape()

            self.argmax_idx = tf.to_int32(tf.argmax(self.softmax_v, axis=1))
            assert self.argmax_idx.get_shape() == [cfg.batch_size, 1, 1], self.argmax_idx.get_shape()
            ...
            if not cfg.mask_with_y:
                ...
                assert self.masked_v.get_shape() == [cfg.batch_size, 1, 16, 1], self.masked_v.get_shape()
            else:
    ...
    def loss(self):
        ...
        assert max_l.get_shape() == [cfg.batch_size, 10, 1, 1], max_l.get_shape()
